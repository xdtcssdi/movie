...
//初始化HTML
            ...
            //初始化巨幕
            initBanner: function () {
                movie_id = this.getUrlParams('movie_id');
                ...
                $.ajax({
                    ...
                    success: function (obj) {
                        ...
                        actionBuyBtn.append("<a class=\"btn buy\" href=\"movieDetail?movie_id=" + movie_id.toString() + "\" data-act=\"more-detail-click\">查看更多电影详情</a>");
            ...
            initTags: function () {
                ...
                DateActive = this.inputTags(DateStr.length, DateActive, date);
                BrandActive = this.inputTags(BrandStr.length, BrandActive, brand);
                AreaActive = this.inputTags(AreaStr.length, AreaActive, area);
                HallActive = this.inputTags(AreaStr.length, HallActive, hall);
            ...
            //导入活跃标签
            inputTags: function (length, Active, tags) {
            ...
            //初始化url参数
            initParams: function () {
                if (this.getUrlParams('date') == null) {
                    date = "0";
                } else {
                    date = this.getUrlParams('date');
                }
                if (this.getUrlParams('brand') == null) {
                    brand = "0";
                } else {
                    brand = this.getUrlParams('brand');
                }
                if (this.getUrlParams('area') == null) {
                    area = "0";
                } else {
                    area = this.getUrlParams('area');
                }
                if (this.getUrlParams('hall') == null) {
                    hall = "0";
                } else {
                    hall = this.getUrlParams('hall');
                }
                if (this.getUrlParams('page') == null) {
                    page = "1";
                } else {
