...
<script>
    ...
    new Vue({
        ...
        data: {},
        mounted() {
            this.initBanner(); //初始化巨幕
        },
        methods: {
            //初始化巨幕
            initBanner: function () {
                let movie_id = this.getUrlParams('movie_id');
                let cinema_id = this.getUrlParams('cinema_id');
                const cinemaBriefContainer = $(".cinema-brief-container");
                const avatarShadow = $(".avatar-shadow");
                $.ajax({
                    type: 'post',
